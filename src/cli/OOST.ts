...
let _splitOptions: SplitOptions = new SplitOptions();
let _stitchOptions: StitchOptions = new StitchOptions();
let _openOrdinalInscriptionsFilePath: string = "";
let _openOrdinalInscriptions: any = [];
 ...
 * @group Main
 */
export async function split(options: SplitOptions): Promise<string[]> {
 ...
 * @group Main
 */
export async function stitch(options: StitchOptions): Promise<void> {
    // Set the global options variable
 ...
 */
async function splitFile(file: string, outPath: string, maxSize: number): Promise<string[]> {
 ...
 */
async function stitchFiles(files: string[], outputFilePath: string): Promise<boolean> {
 ...
 */
async function getTextFromURL(url: string): Promise<string | undefined> {
    // Fetch the response from the given URL
    const response = await fetch(url);
 ...
 */
async function findFileInParentDir(fileName: string, currentDir = import.meta.url): Promise<string | null> {
 ...
 */
async function writeText(filePath: string, data: string): Promise<string> {
 ...
 * @group Logging
 */
export function log(message: string, newLine: boolean = true) {
    if (message.includes("===")) {
 ...
 * @group Logging
 */
export function err(message: string, newLine: boolean = true) {
    if (message.includes("===")) {
        const line = '='.repeat(process.stdout.columns)
        message = chalk.green.bold(line);
    }
 ...
 */
function getInscriptionIdFromUrl(url: string): string {
