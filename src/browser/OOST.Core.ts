...
import { Metadata } from "@open-ordinal/metadata";
...
let _stitchOptions: StitchOptions = new StitchOptions();
let _baseUrl = "";
let _id = "";
 ...
 */
export async function stitchAsArrayBuffer(options: StitchOptions): Promise<ArrayBuffer | undefined> {
    ...
    if (options?.parts) {
        ...
        if (metadata.stitch) {
            if (metadata.stitch.parts) {
                if (metadata.stitch.parts.length == 0) return undefined;
 ...
 */
export async function stitchAsDataUrl(options: StitchOptions): Promise<string | undefined> {
    _stitchOptions = options;
    ...
    if (options?.parts) {
        ...
        if (parts.length != options.parts?.length) return undefined;
        if (options.contentType) {
            contentType = options.contentType;
        }
    } else if (options?.sat) {
 ...
 * @returns {Promise<string | undefined>} A promise that resolves to a string if successful, or undefined if not.
 */
export async function stitchAsString(options: StitchOptions): Promise<string | undefined> {
    _stitchOptions = options;
 ...
 */
function setId(id: string): void { _id = id; }
 ...
 */
async function getInscriptionMetadata(inscriptionId = getId(), baseUrl = _baseUrl): Promise<Metadata> {
    ...
    const data = decode(dataAsBuffer) as Metadata;
 ...
 */
function projectAsContentUrl(id: string): string {
 ...
 */
async function fetchUrlAaString(url: string): Promise<string | undefined> {
 ...
 * @throws {Error} If the fetch operation fails.
 */
async function fetchUrlAsBuffer(url: string): Promise<ArrayBuffer | undefined> {
    const response = await fetch(url);
    if (!response.ok) {
        throw new Error(`Failed to fetch ${url}: ${response.statusText}`);
    }
 ...
 */
function arrayBufferToDataURL(buffer: ArrayBuffer, contentType: string): Promise<string> {
